...
|------|---------|------------|
| [`create-todo`](#1-create-todo---create-a-new-todo-with-auto-assigned-task-number) | Create single task with auto-assigned number | `title` (string, required, min 1 char), `description` (string, required, min 1 char) |
...
| [`get-next-todo-number`](#7-get-next-todo-number---get-only-the-next-task-number) | Get just the task number of next incomplete task | none |
| [`bulk-add-todos`](#8-bulk-add-todos---create-multiple-tasks-from-folder-contents) | Create tasks by reading file contents from folder | `folderPath` (absolute path, required, must exist) |
...
Creates a single todo item with automatic integer ID assignment and task numbering.
...
#### 7. `get-next-todo-number` - Get only the next task number
...
### **Token Efficiency**
- **43% fewer tools** (8 vs 14) means significantly lower token usage

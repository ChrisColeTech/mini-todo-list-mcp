...
import { CreateTodoSchema, UpdateTodoSchema, CompleteTodoSchema, DeleteTodoSchema, BulkAddTodosSchema } from "./models/Todo.js";
// Import services
 ...
 */
server.tool("create-todo", "Create a new todo item with auto-assigned task number", {
 ...
 */
server.tool("get-todo", "Get a specific todo by ID", {
 ...
 */
server.tool("update-todo", "Update a todo title or description", {
    id: z.number().int().positive("Invalid Todo ID"),
 ...
 */
server.tool("complete-todo", "Mark a todo as completed", {
    id: z.number().int().positive("Invalid Todo ID"),
}, async ({ id }) => {
    const result = await safeExecute(() => {
 ...
 */
server.tool("delete-todo", "Delete a todo", {
    id: z.number().int().positive("Invalid Todo ID"),
}, async ({ id }) => {
    const result = await safeExecute(() => {
 ...
 */
server.tool("get-next-todo", "Get the next todo that needs to be completed (status != 'Done')", {}, async () => {
    const result = await safeExecute(() => {
 ...
 */
server.tool("get-next-todo-number", "Get only the task number of the next incomplete todo", {}, async () => {
    const result = await safeExecute(() => {
 ...
 */
server.tool("bulk-add-todos", "Create todos by reading file contents from a folder (recursively scans all files)", {
 ...
 */
server.tool("clear-all-todos", "Delete all todos from the database", {}, async () => {
    const result = await safeExecute(() => {
